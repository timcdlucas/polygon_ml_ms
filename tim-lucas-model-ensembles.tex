...
These predictions were finally empirical logit transformed so that they were on the linear predictor scale of the top level model.
...
We used the environmental and anthropogenic covariates, centered and standardised, as a baseline model (subsequantly called Enviro).
...
First we used only the predictions from the machine learning models trained on local data (subsequantly called ML\textsubscript{l}).
Secondly, we combined the predictions from the machine learning models and the environmental coviarates in one model (Enviro + ML\textsubscript{l}).
...
We also combined predictions from the machine learning models trained on regional data and global data (ML\textsubscript{l} + ML\textsubscript{g}).
