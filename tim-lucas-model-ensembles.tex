...
\caption{
  Schematic of the baseline disaggregation regression model and the two stage method. 
  Models are shown in yellow ovals, malaria data is shown in purple rectangles and covariates are shown in green rectangles.
  The baseline model uses aggregated incidence data and raw environmental covarates in a disaggregation regression model.
  In the two stage method, new covariates are created in stage 1 by training machine learning models on prevalence data.
...
The experiments that follow in this paper assess the improved performance of disaggregation regression models when using the predictions from these machine learning models as covariates as compared to a baseline disaggregation regression model using only the raw environmental covariates.
