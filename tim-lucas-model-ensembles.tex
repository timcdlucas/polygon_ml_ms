...
Here we propose training a suite of level-zero machine learning models on point-level, binomial prevalence data and combining these predictions by using them as covariates in a polygon-level, Poisson incidence model.
...
This scheme is also similar to applications in other disease contexts where data from vector or host species are used to train models, the predictions from which are then used in a final model \citep{pigott2014mapping, shearer2016estimating}.
In those applications we would always expect to need additional covariates as well the modelled distributions of hosts or vectors while in this application, because both sets of data are direct measures of some aspect of malaria transmission rate, it is possible that we would need no further covariates. 
...
The author[oxford]s acknowledge the National Malaria Control Programme of Madagascar for sharing their routine case data for this analysis.
